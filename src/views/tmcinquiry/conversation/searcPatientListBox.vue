<template>
  <div id="searcPatientListBox" v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0)">
    ...
    <div v-if="!timeKeys.length&&!loading" style="text-align: center;line-height: 60px;font-size: 14px;color: #8e949d">暂无搜索结果</div>
  </div>
</template>
...
export default {
  name: "dropDownList",
  mixins: [dealListMsg],
  props: ['content', 'timeAndMultiTalkProps', 'timeKeysProps'],
  computed: {
  ...
  methods: {
    ...
    //   let _this = this;
    //   if (this.content) {
    //     for (let key in this.timeAndMultiTalkProps) {
    //       const ele = this.timeAndMultiTalkProps[key]
    //       if (
    //         (!this.isEmptyObject(ele.tmcPatient) && ele.tmcPatient.patientName && ((ele.tmcPatient.patientName.indexOf(_this.content)) != -1)) ||
    //         (!this.isEmptyObject(ele.patient) && ele.patient.personName && ((ele.patient.personName.indexOf(_this.content)) != -1))
    //       ) {
    //         obj[key] = ele
    //       }
    //     }
    //   }
    //   this.timeAndMultiTalk = obj;
    ...
    getPatientList () {
      ...
      let _this = this;
      this.$_http
        .get(this.$_API.INTERFACE_TMC_MULTITALK_QUERYPAGE, {
          params
        })
        .then(res => {
