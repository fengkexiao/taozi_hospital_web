  ...
  <div class="app-container base-page">
    <div>
      ...
      <div class="base-search_form">
        ...
        <div class="form-item">
          ...
          <el-date-picker v-model="rpDate" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" clearable>
          </el-date-picker>
        </div>
        <div class="form-item">
          <span class="item-label">处方ID：</span>
          <el-input v-model="query.rpID" placeholder="请输入处方ID" clearable></el-input>
        </div>
        <div class="form-item">
          <span class="item-label">患者：</span>
          <el-input v-model="query.patientName" placeholder="请输入患者" clearable></el-input>
        </div>
        <div class="form-item">
          <span class="item-label">医生：</span>
          <el-input v-model="query.doctorName" placeholder="请输入医生" clearable></el-input>
        </div>
        <div class="form-item">
          <span class="item-label">处方状态：</span>
          <el-select v-model="query.rpStatus" placeholder="请选择" clearable filterable>
    ...
    <div>
      ...
      <el-table :data="tableData" stripe :loading="tableLoading" class="mt24">
        ...
        <el-table-column prop="pharmacistName" label="药师名称" align="center" width="155px"></el-table-column>
        ...
        <el-table-column prop="notPassReason" label="不通过原因" align="center" min-width="125"></el-table-column>
        <el-table-column label="操作" align="center" fixed="right">
          <template #default="{row}">
            <el-button type="text" size="mini" @click="trialParty(row, '1')">
              详情
            </el-button>
          </template>
        </el-table-column>
      </el-table>
      <base-pagination background v-model="pageInfo" @size-change="loadTable" @current-change="loadTable" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    loadTable (key) {
      ...
      let params = { ...this.pageInfo, ...this.query, ...rpdate, pharmacistID: this.userInfo.doctorID || "" };
      this.tableLoading = true;
      // this.$_API.API_RP_PRESCRIPTION_GETRP
      let requests = this.$_http.post(this.$_API.INTERFACE_RP_GET_HISTORYRPOFPHAR,
        params
      );
      requests
        .then(res => {
