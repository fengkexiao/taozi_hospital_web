<template>
  <div class="pt0">
    ...
    <el-table :data="list" ref="multipleTable" style="width:100%;" :loading="tableLoading" class="mt24" stripe  @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="medicineContentA" label="A药品成分" :show-overflow-tooltip="true"  align="center">
        <template slot-scope="scope">
          {{scope.row.interactionClass===1?scope.row.medicineContentA:""}}
        </template>
      </el-table-column>
      <el-table-column prop="medicineContentA" label="A成分类型" :show-overflow-tooltip="true" align="center">
        <template slot-scope="scope">
          {{scope.row.interactionClass===2?scope.row.medicineContentA:""}}
        </template>
      </el-table-column>
      <el-table-column prop="medicineContentB" label="B药品成分" :show-overflow-tooltip="true" align="center">
        <template slot-scope="scope">
          {{scope.row.interactionClass===1?scope.row.medicineContentB:""}}
        </template>
      </el-table-column>
      <el-table-column prop="medicineContentB" label="B成分类型" :show-overflow-tooltip="true" align="center">
        <template slot-scope="scope">
          {{scope.row.interactionClass===2?scope.row.medicineContentB:""}}
        </template>
      </el-table-column>
    ...
    <medicine-interaction-edit :title="!isEdit?'药品名称作用规则添加':'药品名称作用规则编辑'" :visible.sync="dialogVisible" type="A" :isEdit="isEdit" :selData="selData"></medicine-interaction-edit>
  </div>
</template>
