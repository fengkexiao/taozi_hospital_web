<template>
  <div class="app-container base-page">
    ...
    <base-title name="检查检验结算列表"></base-title>
    <el-table :data="orderData" stripe class="mt24" :loading="tableLoading">
      ...
      <el-table-column prop="platformIncome" label="应付平台服务费（元）" align="center" width="180px">
        <template #default="{row}">
          {{row.platformIncome | formatMoney() }}
        </template>
      </el-table-column>
      <el-table-column prop="doctorIncome" label="应付推荐医生服务费（元）" align="center" width="200px">
        <template #default="{row}">
      ...
      <el-table-column label="付款时间">
        <template #default="{row}">
          <div class="option-btn_group">
            <span class="opt-text" v-if="row.hospitalCostTime=='1900-01-01 00:00:00'">暂未付款</span>
            <span class="opt-text" v-if="row.hospitalCostTime!='1900-01-01 00:00:00'">{{row.hospitalCostTime}}</span>
          </div>
        </template>
      </el-table-column>
    </el-table>
