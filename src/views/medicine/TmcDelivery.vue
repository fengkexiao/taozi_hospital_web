<template>
  <div class="app-container base-page" id="detailBody">
    ...
    <base-title name="药房发货列表"></base-title>
    <el-table :data="tableData" stripe :loading="tableLoading" class="mt24">
      ...
      <el-table-column prop="18" label="操作" width="180" align="center" fixed="right">
...
export default {
  ...
  methods: {
    ...
    confirmDelivery (row) {
      this.$router.push({
        name: "to-Examine",
        params: { orderID: row.keyID, sign: "2", route: "tmc-tmcDelivery" }
      });
    },
    // 详情
    details (row) {
      this.$router.push({
        name: "to-Examine",
        params: { orderID: row.keyID, sign: "3", route: "tmc-tmcDelivery" }
      });
    },
    // 返回修改状态
    cancelStates () {
